...
##???Consider other normalization methods, like RUVs
...
cont.matrix <- makeContrasts(ShamvsSCI=GA-GB,
                             levels=design)
...
complete_list <- topTable(fit2, number=Inf, adjust="BH", coef=NULL)
...
complete_list <- rownames_to_column(complete_list, var="Gene ID")
#This will make the ensembl rownames into a column! So that it can be name matched 
mart = useMart('ensembl')
##This uses the package biomaRt to eventually convert ensemble ID to gene name
...
gene <- getBM( attributes = c("ensembl_gene_id","external_gene_name"),values = complete_list$`Gene ID`,mart = ensembl)  
#Now we need to match the gene id with ensembl_gene_id
id <- match(complete_list$`Gene ID`, gene$ensembl_gene_id)
#Add Gene symbol column into the data frame
complete_list$`Gene ID` <- gene$external_gene_name[id]
head(complete_list)
